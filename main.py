import json
from difflib import get_close_matches

def load_knowledge_base(file_path: str):
    with open(file_path, 'r') as file:
        data: dict = json.load(file)
    return data

def save_knowledge_base(file_path: str, data: dict):
    with open(file_path, 'w') as file:
        json.dump(data, file, indent=2)

def find_best_match(user_question: str, questions: list[str]) -> str | None:
    matches: list = get_close_matches(user_question, questions, n=1, cutoff=0.6)
    return matches[0] if matches else None

def get_answer_for_question(question: str, knowledge_base: dict) -> str | None:
    for q in knowledge_base["questions"]:
        if q["question"] == question:
            return q["answer"]
    return None

def log_conversation(user_input: str, bot_response: str):
    with open('conversations.log', 'a') as log_file:
        log_file.write(f'User: {user_input}\nBot: {bot_response}\n\n')

def show_help():
    print("Welcome to the Medical Center ChatBot.")
    print("You can ask questions about location, hours, scheduling appointments, and more.")
    print("If I don't understand your question, I'll ask you to teach me.")

def collect_feedback(feedback: str):
    with open('feedback.log', 'a') as feedback_file:
        feedback_file.write(f'Feedback: {feedback}\n')

def chat_bot():
    knowledge_base: dict = load_knowledge_base('knowledge_base.json')
    
    while True:
        user_input: str = input('You: ')
        
        if user_input.lower() == 'quit':
            break
        
        best_match: str | None = find_best_match(user_input, [q["question"] for q in knowledge_base["questions"]])
        
        if best_match:
            answer: str | None = get_answer_for_question(best_match, knowledge_base)
            if answer:
                print(f'Bot: {answer}')
                log_conversation(user_input, answer)
            else:
                print(f'Bot: I don\'t know the answer. Can you teach me?')
                new_answer: str = input('Type the answer or "skip" to skip: ')
                if new_answer.lower() != 'skip':
                    knowledge_base["questions"].append({"question": best_match, "answer": new_answer})
                    save_knowledge_base('knowledge_base.json', knowledge_base)
                    print('Bot: Thank you for teaching me!')
                    collect_feedback(f'Taught a new answer: {best_match}')
        else:
            print(f'Bot: I don\'t understand the question. Can you reformulate it or provide more details?')
            new_answer: str = input('Type the answer or "skip" to skip: ')
            if new_answer.lower() != 'skip':
                knowledge_base["questions"].append({"question": user_input, "answer": new_answer})
                save_knowledge_base('knowledge_base.json', knowledge_base)
                print('Bot: Thank you for teaching me!')
                collect_feedback(f'Taught a new question: {user_input}')

if __name__ == "__main__":
    show_help()
    chat_bot()
